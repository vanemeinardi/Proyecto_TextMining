...
tabla=pd.read_csv('/home/vmeinardi/Proyecto_NLP/Variables_contextuales_comunidades.csv')
print(tabla.columns)
...
# Crear el pipeline que combine el preprocesamiento y el modelo de árbol de decisión
pipeline = Pipeline(steps=[
    ('preprocessor', preprocessor),
    ('classifier', DecisionTreeClassifier(random_state=42))
])

# Definir los parámetros a probar en el árbol de decisión
param_grid = {
...
data = {
    'Parámetros': [f"Max Depth={params['classifier__max_depth']}, Criterion={params['classifier__criterion']}, Min Samples Split={params['classifier__min_samples_split']}" for params in results['params']],
...
results_df.to_csv("decision_tree_results_caracteristicas_contextuales_comunidades.csv", index=False)
...
metrics_df.to_csv("DT_best_parametros_caracteristicas_contextuales_comunidades.csv")
